...
export default function FAQs() {
  return <div className="bg-white flex flex-col items-start relative w-full">
      {/* FAQ */}<main className="bg-[#fefcf5] overflow-clip relative shrink-0 w-full">
      <Header />
      <div className="absolute inset-[97.23%_0_0_0]">
      <FAQsSvgs.FAQsSvg01 />
    </div>
      <SectionShell className="bg-[#fefcf5]" containerClassName="grid gap-12 lg:grid-cols-[minmax(260px,360px)_minmax(0,1fr)] lg:gap-[72px] lg:pl-[120px] xl:pl-[160px]" size="wide" spacing="hero">
      ...
      <p className={`${typeStyles.bodyLead} font-medium leading-normal relative shrink-0 text-[#0f251b] whitespace-nowrap`}>
        Have more questions?
      </p>
      <Link to="/contact-us" className="bg-green-500 flex items-start px-4 py-2 relative rounded-[40px] shrink-0">
        <p className={`${typeStyles.body} relative shrink-0 text-[#eee] text-center whitespace-nowrap`}>
          Contact Us
        </p>
      </Link>
    </div>
      ...
      {faqFilters.map((filter, index) => <div className={`flex items-start px-4 py-2 relative rounded-[40px] shrink-0 ${index === 0 ? "bg-green-500" : "bg-[#eff6ef]"}`} key={filter}>
          <p className={`${typeStyles.body} relative shrink-0 text-center whitespace-nowrap ${index === 0 ? "text-[#eee]" : "text-[#0f251b]"}`}>
            {filter}
          </p>
        </div>)}
    </div>
          <div className="flex flex-col gap-px items-start relative w-full">
      {faqItems.map(question => <div className="flex gap-6 items-center py-4 relative shrink-0 w-full md:gap-16" data-name="faq-card" key={question}>
      ...
      <div className="flex flex-[1_0_0] flex-col gap-4 items-start min-w-px relative">
        <p className={`${typeStyles.cardTitle} leading-copy relative shrink-0 text-[#0f251b] w-full`}>
          {question}
        </p>
      </div>
      <div className="overflow-clip relative shrink-0 size-6" data-name="plus">
      ...
      <p className={`${typeStyles.bodyLead} font-medium leading-normal relative shrink-0 text-[#0f251b]`}>
        Have more questions?
      </p>
      <Link to="/contact-us" className="bg-green-500 flex items-start px-4 py-2 relative rounded-[40px] shrink-0">
        <p className={`${typeStyles.body} relative shrink-0 text-[#eee] text-center whitespace-nowrap`}>
